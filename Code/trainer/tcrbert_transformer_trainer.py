...
class TCRBertTrainer(BaseTrainer):
    """
    Trainer class
    """
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (x_input_ids, x_attention_mask, target) in enumerate(self.data_loader):
            ...
            output = self.model(x_input_ids, x_attention_mask, torch.squeeze(target)[:, :-1])
            # target shape: [batch_size, seq_len*2], output shape: [batch_size, seq_len*2, n_token]
            loss = self.criterion(output.view(-1, self.ntoken), torch.flatten(torch.squeeze(target)[:, 1:]))
            loss.backward()
            self.optimizer.step()
            ...
            with torch.no_grad():
                ...
                y_true = torch.squeeze(target)[:, 1:].cpu().detach().numpy()
                for met in self.metric_fns:
                    self.train_metrics.update(met.__name__, met(y_pred, y_true))

                # compute the total correct predictions
                correct, num = correct_count_seq(y_pred, y_true)
    ...
    def test(self):
        ...
        test_output = {'n_samples': len(self.test_data_loader.sampler) * output.shape[1],
